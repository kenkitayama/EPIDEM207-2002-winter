...
PROC CONTENTS DATA=project1.project1B_1; 
	TITLE 'Data Dictionary for Kim et al. 2021';
RUN;
...
QUIT;
ODS tagsets.excelxp FILE="C:\Users\Ken Kitayama\Dropbox\Dropbox\UCLA FSPH\2021-2022\Winter 21-22\EPI 209 - Reproducibility in Epidemiologic Research\Assignments\Assignment 1\Data Dictionary Project1B.xls" style=statistical;
...
/* Used macro from: https://blogs.sas.com/content/sgf/2021/02/01/creating-codebooks-with-sas-macros/ */

title 'Codebook for Kim et al. 2021';
title2 'Association between sarcopenia level and metabolic syndrome';

options papersize=letter orientation=portrait leftmargin=.5in rightmargin=.5in topmargin=.5in bottommargin=.5in;
options label nodate nonumber ;

*-----------------------UPDATE THE FOLLOWING AS NEEDED---------------------------------------------------;
%let TKFolder = C:\Users\Ken Kitayama\Dropbox\Dropbox\UCLA FSPH\2021-2022\Winter 21-22\EPI 209 - Reproducibility in Epidemiologic Research\Assignments\Assignment 1\Codebook\74013_example.zip\TKFolder;
%let DataFolder = C:\Users\Ken Kitayama\Dropbox\Dropbox\UCLA FSPH\2021-2022\Winter 21-22\EPI 209 - Reproducibility in Epidemiologic Research\Assignments\Assignment 1;
%let WorkFolder = C:\Users\Ken Kitayama\Dropbox\Dropbox\UCLA FSPH\2021-2022\Winter 21-22\EPI 209 - Reproducibility in Epidemiologic Research\Assignments\Assignment 1\Codebook;

libname SAS_data "&DataFolder";
options fmtsearch=(project1);  /* Look for the format.sas7bdat catalog of formats in the folder assigned to sas_data*/

%let cb_name=&WorkFolder/Project1B_Codebook.rtf ;
%include "&TKFolder./TK_codebook.sas"; *<--Codebook program;
*----------------------------------------------------------------------------------------------------------;
...
/* TK_codebook maco */
/* Again, see macro from: https://blogs.sas.com/content/sgf/2021/02/01/creating-codebooks-with-sas-macros/ */
...
CB_TYPE = type of codebook (XML, XLSX, PDF, RTF)
...
User Specified:  all TITLES and all FOOTNOTES are specified by user.
...
%macro tk_codebook(lib,file1=, fmtlib=, var_order=, cb_type=, cb_file=, cb_output=, organization=, include_warn=YES, cb_size=FULL);
    ...
    data _cb_freqs_;
        ...
        /*         %IF %UPCASE(%SCAN(table, 2, ' ')) EQ "PERCENT"  %THEN
                ...
                %END ;
        */
        if upcase(scan(table,2,' ')) = "PERCENT" and not missing(percent2) then
            percent=percent2;
        else if upcase(scan(table,2,' ')) = "FREQUENCY" and not missing(frequency2) then
            frequency=frequency2;
        else if upcase(scan(table,2,' ')) = "CUMFREQUENCY" and not missing(cumfrequency2) then
            cumfrequency=cumfrequency2;

        if upcase(scan(table,2,' ')) = "CUMPERCENT" and not missing(cumpercent2) then
            cumpercent=cumpercent2;
    ...
    ** SECTION 8 - Merge the Format Ranges, based on format and format label (desc),
        ...
        if a = 1 and b = 1 and has_freq_results=1 then
            ...
            do;
                ...
                if compress(desc) in ('.') then
                    desc='SAS missing (.)';             
                else if compress(desc) in (' ') and compress(type)='Num' then
                    desc='SAS missing (.)';                
     ...
     *desc=strip(desc);*/
        ...
        if min = max and n > 0 and not missing(compress(range)) and not missing(min) and not missing(max) then
            do;
                warning = 'POTENTIAL PROBLEM IN DATA: All nonmissing values are the same for these variables ';
                put 'Warning: All nonmissing values are the same for: ' name=;
                output _cb_msgs3_;
            end;
    ...
    %else
        ...
        %end;
            ...
            data _cb_seq_data_;
                ...
                if compress(desc)="SASmissing(.)" then
                    do;
                        %if %upcase(&cb_type) = XLSX %then
                            %do;
                                range=' ';  /*EXCEL turns . into 0 so code to blank for now. */
                            %end;
                        %else
                            %do;
                                range='.';
                ...
                if compress(desc)="Missing(blank)" then
                    range=' ';

                if compress(desc)='Range(MintoMax)' and compress(range)='Range(MintoMax)' then
                    range=' ';
            run;
            ...
            %if "&cb_output" ^= "" %then
                %do;
                    ...
                    data &cb_output (label="Codebook data set for &file1");
                        retain set_name set_label date_created num_obs num_var order name label format type_length mean_char order_flag range cnt desc frequency percent CumFrequency CumPercent;
                        ...
                        label NUM_OBS = "Number of observations in data set";
    ...
    proc report data=_cb_seq_data_ nocenter nowindows headline wrap split='~'  missing
        ...
        %if %upcase(&cb_size) = BRIEF %then
            %do;
                ...
                define cnt / order noprint;
                define desc  / group center flow  style(column)=[width=10.5%] style(column)={tagattr="format:@"};
...
%Macro CB_PrintNoVariation(cbvary);
    ...
    proc report data=&cbvary nocenter nowindows headline wrap split='~'  missing
        style(header)=[color=black backgroundcolor=very light grey ]
        style(summary)=[color=very light grey backgroundcolor=very light grey fontfamily="Times Roman" fontsize=1pt textalign=r];;
        column ( "POTENTIAL PROBLEM: NUMERIC VARIABLES WITH NO VARIATION IN RESPONSE~All nonmissing values are the same for these variables"
...
%macro CB_PrintMissFmtN(cbnofmtn);
    ...
    proc report data=&cbnofmtn nocenter nowindows headline wrap split='~'  missing
        style(header)=[color=black backgroundcolor=very light grey ]
        style(summary)=[color=very light grey backgroundcolor=very light grey fontfamily="Times Roman" fontsize=1pt textalign=r];;
        column ("POTENTIAL PROBLEM: NUMERIC VARIABLES NOT ASSIGNED USER-DEFINED FORMAT~Codebook uses categories 'Range', 'SAS Missing (.)'~Assign format with categories 'Valid Range', 'Missing'~Values outside of Valid Range will be identified in Out of Range Report."
            name type length label);
        /*column ("POTENTIAL PROBLEM: NUMERIC VARIABLES NOT ASSIGNED A USER-DEFINED FORMAT~Codebook uses categories 'Range', 'SAS Missing (.) if no format is assigned'~Assign format with categories 'Valid Range', 'Missing'~Values outside of Valid Range will be identified in Out of Range Report."
            name type length label);*/
...
%macro CB_PrintNoVarLbl(cbnolbl);
    ...
    data &cbnolbl;
        ...
        if missing(label) then
            potential_problem='Missing variable label';
